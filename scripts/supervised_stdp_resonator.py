...
def search_for_parameters(freq0, thetas, weights):
    duration = 22 / freq0
    ...
    chosen_thetas = thetas_arr[min_mse]
    chosen_weights = weights_arr[min_mse]
    ...
    fig.suptitle(f'{mses[min_mse].mean()}')
    ...
    with open(f"../filters3/clk_{clk_freq}/parameters/f_{int(freq0)}.json", 'w') as best_params_f:
        parameters = {
            ...
            'mse': list(mses[min_mse]),
            'mean_mse': float(mses[min_mse].mean()),
...
freqs = [104 * (.87 ** i) for i in range(35)]

init_thetas = [-1, -5, -5, -5]
init_weights = [11, 9, 10, 10, 10]
for freq in freqs:
    init_thetas, init_weights = search_for_parameters(freq, init_thetas, init_weights)
